...
module.exports = {
	...
	feedbackSchema: require('./schema/feedbackSchema'),
	...
	numbers: {
		...
		async remove(number) {
			const remove = await numberSchema.deleteOne({ number: number });
			return remove;
		},
		...
		async get(numbers) {
			const res = await numberSchema.findOne({ number: numbers });
			return res;
		},
		async list() {
